          ...
          <>
              ...
              <div className="space-y-4 mb-6 bg-yellow-950/20 p-4 rounded-lg border border-yellow-900/30">
                <div>
                  ...
                  >
                    ...
                    <option value="Critical">Critical (Sovereign)</option>
                  </select>
                </div>
              </div>
            )}
            ...
            <div className="bg-slate-900/40 p-6 rounded-[2.5rem] border border-white/5 space-y-6">
              ...
              <div className="pt-4 border-t border-white/5">
                <label className="text-[9px] text-slate-600 block mb-2 font-black uppercase tracking-widest ml-1">System State</label>
                <div className="flex items-center justify-between bg-slate-950/50 rounded-xl px-4 py-3 border border-white/5">
                  <div className="flex items-center gap-3">
                    <div className={`w-2 h-2 rounded-full ${selectedNode.systemState === 'running' ? 'bg-emerald-500 shadow-[0_0_10px_emerald]' : selectedNode.systemState === 'booting' ? 'bg-yellow-500 animate-pulse' : 'bg-slate-600'}`} />
                    <span className="text-[10px] font-black uppercase text-white tracking-widest">{selectedNode.systemState}</span>
                  </div>
                  {selectedNode.systemState === 'booting' && (
                    <span className="text-[10px] font-mono text-teal-400">{selectedNode.bootProgress}%</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 border-t border-slate-800 pt-6">
              <button
                ...
                className="w-full py-3 bg-red-900/40 hover:bg-red-800/60 text-red-400 hover:text-red-300 border border-red-800/50 rounded-xl transition-all text-[10px] font-black uppercase tracking-widest"
              >
                Decommission Device
              </button>
            </div>
              ...
              return activeConns.map(conn => {
                ...
                return (
                  <div key={conn.id} className="bg-slate-800/50 p-4 rounded-lg border border-slate-700/50">
                    ...
                    <div className="w-full bg-slate-950 rounded-sm h-2 overflow-hidden mb-3">
                      <div
                  ...
                  <div key={service.id} className="bg-slate-900/60 border border-slate-800 p-4 rounded-xl flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${service.status === 'running'

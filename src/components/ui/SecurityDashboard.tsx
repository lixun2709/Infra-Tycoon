                  ...
                  {filteredNodes.map(node => {
                    ...
                    return (
                      <Card key={node.id} className={`p-3 flex items-center justify-between transition-colors ${getStatusColor(state)} hover:brightness-110`}>
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            variant={node.microsegmentationEnabled ? 'success' : 'ghost'}
                            onClick={() => toggleMicrosegmentation(node.id, !node.microsegmentationEnabled)}
                            icon={<ShieldCheck size={14} />}
                            className={`h-8 text-[10px] font-black tracking-widest ${node.microsegmentationEnabled ? 'bg-blue-600/20 text-blue-400 border-blue-500/50 hover:bg-blue-600/30' : 'bg-slate-800/50 text-slate-400 hover:text-slate-200 border-slate-700'}`}
                          >
                            MICROSEG
                          </Button>

                          <Button
                            variant={node.isIsolated ? 'ghost' : 'outline'}
                            onClick={() => !node.isIsolated && isolateNode(node.id)}
                            disabled={node.isIsolated}
                            icon={<WifiOff size={14} />}
                            className={`h-8 text-[10px] font-black tracking-widest ${node.isIsolated ? 'opacity-40 cursor-not-allowed bg-indigo-950 text-indigo-500 border-indigo-900' : 'hover:bg-indigo-500/20 hover:text-indigo-400 border-slate-700 text-slate-300 hover:border-indigo-500/50'}`}
                          >
                          ...
                          {isCritical && (
                            <Button
                              variant="danger"
                              onClick={() => formatNode(node.id)}
                              icon={<TerminalSquare size={14} />}
                              className="h-8 text-[10px] font-black tracking-widest ml-2 border-red-500/50"
                            >
                              FORMAT DRIVE
                            </Button>
                          )}
